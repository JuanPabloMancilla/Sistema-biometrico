...
class AccountView(ctk.CTkFrame):
    ...
    def crear_vista_lectura(self):
        ...
        ctk.CTkLabel(title_cont, text="⚙️ Mi Cuenta", font=self.font_header, text_color=COLORS["text"]).pack(anchor="w")
        ctk.CTkLabel(title_cont, text="Configura tu perfil y preferencias", font=self.font_normal, text_color=COLORS["subtext"]).pack(anchor="w")

        ctk.CTkButton(header, text="📝 Editar Perfil", fg_color=COLORS["card"], text_color=COLORS["text"], 
        ...
        ctk.CTkLabel(self.container, text="📋 Detalles de la Cuenta", font=self.font_sub, text_color=COLORS["text"]).pack(anchor="w", padx=100, pady=(20, 10))
    ...
    def create_customization_card(self):
        ...
        ctk.CTkLabel(card, text="🎨 Personalización", font=self.font_sub, text_color=COLORS["text"]).pack(anchor="w", padx=20, pady=(15, 10))
        
        # Fila Modo Oscuro
        f1 = ctk.CTkFrame(card, fg_color="transparent", height=40)
        f1.pack(fill="x", padx=20, pady=5)
        ctk.CTkLabel(f1, text="🌙 Modo Oscuro", font=self.font_normal, text_color=COLORS["text"]).pack(side="left")
        ctk.CTkSwitch(f1, text="", progress_color=COLORS["primary"]).pack(side="right")

        # Fila Idioma
        f2 = ctk.CTkFrame(card, fg_color="transparent", height=40)
        f2.pack(fill="x", padx=20, pady=(5, 20))
        ctk.CTkLabel(f2, text="🌐 Idioma del Sistema", font=self.font_normal, text_color=COLORS["text"]).pack(side="left")
        ...
        ctk.CTkButton(lang_group, text="EN", width=40, height=30, fg_color="transparent", text_color=COLORS["text"], corner_radius=8, font=self.font_small, hover_color="#CBD5E1").pack(side="left", padx=2, pady=2)
    ...
    def abrir_formulario_edicion(self):
        ...
        ctk.CTkLabel(self, text="✏️ Editar Mi Información", font=self.font_header, text_color=COLORS["text"]).pack(anchor="w", padx=60, pady=(40, 20))
        ...
        card = ctk.CTkFrame(form_scroll, fg_color=COLORS["card"], corner_radius=15, border_width=1, border_color=COLORS["border"])
        card.pack(fill="x", pady=10, padx=100)
        ...
        ctk.CTkButton(btns, text="❌ Cancelar", font=self.font_sub, fg_color="#FEE2E2", text_color=COLORS["text"], height=55, 
                    command=self.crear_vista_lectura).pack(side="left", expand=True, fill="x", padx=(0, 10))
        
        ctk.CTkButton(btns, text="💾 Guardar Cambios", font=self.font_sub, fg_color="#D1FAE5", text_color=COLORS["text"], height=55, 
                    command=self.guardar_datos).pack(side="left", expand=True, fill="x", padx=(10, 0))

    def create_input(self, master, label, value):
        ctk.CTkLabel(master, text=label, font=self.font_small, text_color=COLORS["text"]).pack(anchor="w", padx=25, pady=(20, 5))
        entry = ctk.CTkEntry(master, height=45, font=self.font_normal, fg_color=COLORS["hover"], border_width=0, text_color=COLORS["text"])
